...
ylabel('x')
zlabel('y')
print('3D_trajectory_ENKF2','-djpeg')
...
xlabel('x')
ylabel('z')
print('2D_trajectory_ENKF2','-djpeg')
%%
...
nFrames = 471;
vidObj = VideoWriter('lorenz63_3d_plus_ENKF2.avi');
vidObj.Quality = 100;
vidObj.FrameRate = 30;
open(vidObj);
...
for jj=(ll+2):nsteps
    ...
    if(mod(jj,10)==0)
        ...
        hold on
        plot3(KF(3,jj-ll:jj),KF(1,jj-ll:jj),KF(2,jj-ll:jj),'*','Color',Color(:,9),'MarkerSize',3)
        axis([-10 60 -25 25 -30 30])
        hold off
        drawnow()
        writeVideo(vidObj, getframe(gca));
...
nFrames = 471;
vidObj = VideoWriter('lorenz63_2d_plus_ENKF2.avi');
vidObj.Quality = 100;
vidObj.FrameRate = 30;
open(vidObj);
writeVideo(vidObj, getframe(gca));
...
for jj=(ll+2):nsteps
    ...
    if(mod(jj,10)==0)
        ...
        hold on
        plot(KF(1,jj-ll:jj),KF(3,jj-ll:jj),'*','Color',Color(:,9),'MarkerSize',3)
        axis([-25 25 -10 60])
        hold off
        drawnow()
        writeVideo(vidObj, getframe(gca));
...
ylabel('x')
zlabel('y')
print('3D_trajectory_plus_ENKF2','-djpeg')
hold off
...
xlabel('x')
ylabel('z')
print('2D_trajectory_plus_ENKF2','-djpeg')
hold off
