...
for ii=2:nsteps
    Wallaby = lorenz96s4(Wallaby,dt,M,N,F);%+sqrt(dt).*randn(n,1);
end
%%
...
Traj(:,1) = Wallaby;
for ii=2:nsteps
    Traj(:,ii) = lorenz96s4(Traj(:,ii-1),dt,M,N,F);%+sqrt(dt).*randn(n,1);
end
%%

%% observations
Obs = H*Traj +R.*randn(20,nsteps);
%%
...
for jj=2:nsteps
    Ens = lorenz96s4(Ens,dt,M,N,F) + sqrt(dt).*randn(n,Ne);
    for kk=1:Ne
        w(kk) = 0.5.*(Obs(:,jj)-H*Ens(:,kk))'*(eye(20)./R)*(Obs(:,jj)-H*Ens(:,kk));
    end
